...
Moodify is a machine learning project exploring how well audio features can predict four existing emotion labels: Sad, Happy, Energetic, and Calm.

The project explores patterns using unsupervised learning and compares supervised machine learning models with an artificial neural network. XGBoost achieved the highest test accuracy at 96.40%. This measures agreement with the dataset's labels, rather than agreement with independent listener judgments.

The project also includes a Streamlit application that uses the trained XGBoost model to recommend Spotify tracks based on a selected musical mood.
...
Tracks with very high **speechiness** or a duration of **10 minutes or longer** are filtered out to reduce the likelihood of recommending spoken-word content such as podcasts, audiobooks, or stand-up recordings.
...
The project uses the [278k Emotion Labeled Spotify Songs dataset](https://www.kaggle.com/datasets/abdullahorzan/moodify-dataset) from Kaggle.

### Label origin

The emotion labels were already present in the dataset. Available sources suggest that they were generated by the original Moodify project's classifier. See [Analysis Documentation](docs/analysis.md) for label provenance and its implications.
...
Select a musical mood and click **Recommend songs** to receive up to five Spotify recommendations, depending on track availability and the selected filters.
...
The reported accuracy measures agreement with the dataset's existing labels, which appear to be model-generated, rather than independent listener judgments. The model uses audio features only and does not account for lyrics or individual listening experiences.

See the [Analysis Documentation](docs/analysis.md) for details on label provenance, evaluation limitations, and future improvements.
